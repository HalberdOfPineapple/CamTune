...
class TuRBO(BaseOptimizer):
    ...
    def gen_cands_sobol(
        ...
        num_samples = num_samples if num_samples > 0 else self.n_candidates
        # pert - (num_samples, dim)
        pert = sobol.draw(num_samples).to(dtype=self.dtype, device=self.device)
        pert = tr_lbs + (tr_ubs - tr_lbs) * pert

        # Create a perturbation mask
        prob_perturb = min(20.0 / self.dimension, 1.0)
        mask = torch.rand(num_samples, self.dimension, dtype=self.dtype, device=self.device) <= prob_perturb
        ind = torch.where(mask.sum(dim=1) == 0)[0]
        mask[ind, torch.randint(0, self.dimension - 1, size=(len(ind),), device=self.device)] = 1

        # Create candidate points from the perturbations and the mask
        X_cands = region_center.expand(num_samples, self.dimension).clone()
    ...
    def generate_batch(self, 
        ...
        if self.acqf == "ts":
            ...
            # X_cands[mask] = pert[mask]
            X_cands = self.gen_cands_sobol(sobol, x_center, tr_lbs, tr_ubs)
            if path is not None:
                X_cands = X_cands[Node.path_filter(path, X_cands)]
                if len(X_cands) < self.batch_size:
                    print_log(f"[TuRBOComponent] Not enough candidates in the region ({len(X_cands)}/{self.batch_size}), backup sampling", print_msg=True)
                    X_backup = self.gen_cands_sobol(sobol, x_center, tr_lbs, tr_ubs, \
                                                    num_samples=self.batch_size - len(X_cands))
                    X_cands = torch.cat((X_cands, X_backup), dim=0)
        ...
        while not state.restart_triggered and self.num_calls < num_evals:
            ...
            with gpytorch.settings.max_cholesky_size(self.max_cholesky_size):
                try:
                    ...
                    print_log(f"[TuRBOComponent] {ex} happens when fitting model, restart.")
            ...
            Y_next = torch.tensor(
                ...
                [self.obj_func(x) for x in X_next], 
