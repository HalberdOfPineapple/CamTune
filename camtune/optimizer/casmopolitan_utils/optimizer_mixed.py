...
from .localbo_utils import train_gp, from_unit_cube, latin_hypercube, to_unit_cube, ordinal2onehot, onehot2ordinal
...
class MixedOptimizer(Optimizer):
    ...
    def restart(self):
        self.restart_iters.append(len(self.casmopolitan._fX))
        self.tr_records[self.restart_iters[-1]] = {}
        ...
        if self.guided_restart and len(self.casmopolitan._fX):
            ...
            X_init_cat, X_init_cont = X_init[:, :self.n_onehot], X_init[:, self.n_onehot:]
            X_init_cat = from_unit_cube(X_init_cat, self.lb[:self.n_onehot], self.ub[:self.n_onehot])
            if self.warp_discrete:
                X_init_cat = self.warp_discrete(X_init_cat)
            X_init_cat = onehot2ordinal(X_init_cat, self.cat_dims)

            X_init_cont = from_unit_cube(X_init_cont, self.lb[self.n_onehot:], self.ub[self.n_onehot:])

            X_init = np.zeros((self.casmopolitan.n_init, self.true_dim))
            self.X_init[:, self.d_cat] = X_init_cat
            self.X_init[:, self.d_cont] = X_init_cont
            ...
            sobol = SobolEngine(len(self.d_cont), scramble=True, seed=self.seed)
            ...
            self.X_init = np.zeros((self.casmopolitan.n_init, self.true_dim))
            self.X_init[:, self.d_cat] = X_init_cat
            self.X_init[:, self.d_cont] = X_init_cont
    ...
    def suggest(self, n_suggestions=1):
        ...
        if n_adapt > 0:
            if len(self.casmopolitan._X) > 0:  # Use random points if we can't fit a GP
                ...
                X[:, self.casmopolitan.cont_dims] = to_unit_cube(
                    X[:, self.casmopolitan.cont_dims], 
                    lb=self.lb[self.n_onehot:],
                    ub=self.ub[self.n_onehot:])
                fX = copula_standardize(deepcopy(self.casmopolitan._fX).ravel())  # Use Copula
                next, record_dict = self.casmopolitan._create_and_select_candidates(
                    X, fX, length=self.casmopolitan.length_discrete, n_training_steps=300, hypers={})
                next = next[-n_adapt:, :]
                ...
                X_next[-n_adapt:, :] = next
                self.tr_records[self.restart_iters[-1]][len(self.casmopolitan._fX)] = record_dict
