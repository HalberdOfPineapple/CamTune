...
from .localbo_utils import (train_gp, interleaved_search, from_unit_cube, 
                            latin_hypercube, to_unit_cube, onehot2ordinal, 
...
class CASMOPOLITANMixed(CASMOPOLITANCat):
    ...
    *Note*: in general, you should have the first d_cat dimensions as cat_dims, and the rest as the continuous dims.
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        ...
        with gpytorch.settings.max_cholesky_size(self.max_cholesky_size) and torch.enable_grad():
        ...
        record_dict = {
            ...
            'length': self.length,
            'length_discrete': length,
            'succ_counter': self.succcount,
            'fail_counter': self.failcount,
        }
        ...
        def thompson(n_cand=5000):
            ...
            sobol = SobolEngine(len(self.cont_dims), scramble=True, seed=self.seed)
        ...
        # Note the `length` below refers to the discrete length
        if self.acq in ['ei', 'ucb']:
            if self.batch_size == 1:
                if self.acq == 'ei':
                    X_next, acq_next = interleaved_search(x_center[0], _ei,
                                                          self.cat_dims, self.cont_dims,
                                                          self.config, ub, lb, length, self.seed, 3, self.batch_size,
                                                          interval=1)
                else:
                ...
                for p in range(self.batch_size):
                    if self.acq == 'ei':
                        x_next, acq = interleaved_search(x_center[0], _ei,
                                                         self.cat_dims, self.cont_dims,
                                                         self.config, ub, lb, length, self.seed, 3, 1, interval=1)
                    else:
        ...
        if return_acq:
            return X_next, acq_next, record_dict
        return X_next, record_dict
