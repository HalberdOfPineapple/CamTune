...
class Classifier():
    ...
    # propose_samples_turbo -> propose_rand_samples_sobol -> get_sample_ratio_in_region
    def propose_rand_samples_sobol(self, nums_samples, path, lb, ub):
        ...
        if len(final_cands) > nums_samples:
            print(f"[Classifier] Enough samples ({nums_samples}) generated within the region")
            final_cands_idx  = np.random.choice( len(final_cands), nums_samples )
            return final_cands[final_cands_idx]
        else:
            print(f"[Classifier] No enough samples ({len(final_cands)} / {nums_samples}) generated within the region")
    ...
    def propose_samples_turbo(self, num_samples, path, func, batch_size=1, local_num_init=10):
        # throw a uniform sampling in the selected partition
        num_init = local_num_init

        print(f'[Classifier.propose_samples_turbo] Try to propose {num_init} samples within the region...')
        X_init = self.propose_rand_samples_sobol(
            num_init, path, func.lb, func.ub)
