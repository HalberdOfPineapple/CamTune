...
class BaseClassifier:
    def __init__(self, bounds: torch.Tensor, seed: int, classifier_params: Dict) -> None:
        ...
        cluster_method = classifier_params.get('cluster_method', 'kmeans')
        self.init_cluster(cluster_method)
    ...
    def init_cluster(self, cluster_method: str):
        if cluster_method.lower() == 'kmeans':
            from sklearn.cluster import KMeans
            self.clusterizer = KMeans(n_clusters=2, n_init='auto', random_state=self.seed)
        # elif cluster_method.lower() == 'dbscan':
        #     from sklearn.cluster import DBSCAN
        #     self.clusterizer = DBSCAN(eps=0.3, min_samples=2)
        else:
    ...
    def cluster(self, X: torch.Tensor, Y: torch.Tensor):
        ...
        self.clusterizer = self.clusterizer.fit(X_and_Y)
        labels = self.clusterizer.predict(X_and_Y) # (num_samples, )
...
class SVMClassifier(BaseClassifier):
    def __init__(self, bounds: torch.Tensor, seed: int, classifier_params: Dict) -> None:
        super().__init__(bounds, seed, classifier_params)
        self.classifier = SVC(
