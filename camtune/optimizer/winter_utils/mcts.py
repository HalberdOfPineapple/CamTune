...
from camtune.utils.paths import get_tree_dir, get_expr_name
...
class MCTS:
    def __init__(
        ...
        self.default_conf = default_conf # Note that the default configuration here has already been normalized
    ...
    def init_svm_save_dir(self):
        log_idx = 0
        svm_save_dir = os.path.join(get_tree_dir(), get_expr_name())
        while os.path.exists(svm_save_dir):
            log_idx += 1
            svm_save_dir = os.path.join(get_tree_dir(), f'{get_expr_name()}_{log_idx}')
        self.svm_save_dir = svm_save_dir

    def extern_init(self, X: np.array, Y: np.array):
    ...
    def build_tree(self):
        ...
        for node in Node.node_map.values():
            if len(node.children) == 0:
                Node.leaf_nodes.append(node)
                Node.leaf_total_scores += self.get_node_score(node)
